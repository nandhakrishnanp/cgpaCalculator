...
export const SubjectList = ({filtered}:any) => {
    ...
    const subjectArray = filtered[0].subjects
  ...
  return (
    <> {
       ...
       <p>{index+1}/{credits.length}</p>
            <div className='   m-2  rounded-lg bg-white md:shadow-xl flex flex-col p-10 md:p-20 max-md:w-full  my-2 items-center justify-evenly' key={index}>
                <h1 className=' font-semibold text-2xl '>{credits[index].name}</h1>
                <p className=' text-center px-3 '>{credits[index].credits} Credits</p>
                ...
                }} value={credits[index].grade}  type='text' placeholder='Enter Grade eg: A , O ' className=' uppercase ring-2 ring-black h-10 rounded-lg my-2 px-3 ' />
    <div className=' py-2'>

        <button onClick={()=>{
            setIndex(0)
             setCredits(creditArray)
             }} className=' bg-black text-lg hover:scale-105 transition-all duration-150 text-white py-1 m-2 px-3 rounded-full'>Reset</button>
       
    <button disabled={!credits[index].grade} onClick={()=>{
        if(index<credits.length-1){
          ...
          }} className=' bg-black text-lg  hover:scale-105 transition-all duration-150 text-white py-1 m-2 px-3 rounded-full'>{index<credits.length-1?"Next":"Calculate"}</button>
    </div>
            </div>
        </>:<div>
            <h1 className=' text-3xl px-3' >Your  Cummulative  
                grade point  is <b>{cgpa.toFixed(2)}</b> ⚡❤️‍🔥
            </h1>
